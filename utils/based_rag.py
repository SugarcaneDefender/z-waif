...
# Histories
histories_word_id_database = {"me": [], "her": [], "scores": []}
...
history_demarc = (
    20  # This is the point where the history gets considered as usable for RAG
)
...
def run_based_rag(message, her_previous):
    ...
    while j < len(highest_scores):
        ...
        while i < len(history_word_ids):
            if history_word_scores[i] > highest_scores[
                j
            ] and not highest_score_ids.__contains__(history_word_ids[i]):
    ...
    i = 0
    while i < len(histories_word_id_database["me"]):
        score_value = evaluate_message(
            highest_score_ids, histories_word_id_database["me"][i]
        ) + evaluate_message(highest_score_ids, histories_word_id_database["her"][i])
    ...
    current_rag_message += (
        char_name + ": " + history_database[best_message_id - 1][1] + "\n"
    )
    current_rag_message += "User: " + history_database[best_message_id][0] + "\n"
    current_rag_message += (
        char_name + ": " + history_database[best_message_id][1] + "\n"
    )
    current_rag_message += "User: " + history_database[best_message_id + 1][0] + "\n"
    current_rag_message += (
        char_name + ": " + history_database[best_message_id + 1][1] + "\n"
...
def parse_words_to_database(message, flag):
    ...
    while i < len(refined_message):
        ...
        if i + 1 == len(refined_message) or refined_message[i + 1] == " ":
            ...
            if not word_found and count_to_total:
                ...
                word_database["value"].append(
                    0.99
                )  # Note: will have to be recalculated later on for new words
            ...
            if count_to_total:
                word_database["total_word_count"] = (
                    word_database["total_word_count"] + 1
                )
    ...
    if flag == 1:
        ...
        histories_word_id_database["scores"].append(
            0
        )  # Just here so we can score later
...
def prune_common(point):
    ...
    while i < len(histories_word_id_database["me"][point]):
        ...
        if (
            word_database["count"][word_id] / word_database["total_word_count"]
        ) > 0.00077:
...
def word_value_passive_calculation() -> NoReturn:
