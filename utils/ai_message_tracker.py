...
recent_ai_messages: Dict[str, Set[str]] = {}  # platform_channel -> set of recent message hashes
...
# Configuration
MESSAGE_COOLDOWN_SECONDS = 3  # Minimum time between AI responses in same channel
...
def should_ai_respond(message_content: str, platform: str, channel: str) -> bool:
    ...
    if channel_key in recent_ai_messages:
        if message_hash in recent_ai_messages[channel_key]:
            log_info(f"Skipping AI response - message appears to be our own: {message_content[:50]}...")
            return False
    ...
    if channel_key in cooldown_timers:
        ...
        if time_since_last < MESSAGE_COOLDOWN_SECONDS:
            log_info(f"Skipping AI response - cooldown active ({MESSAGE_COOLDOWN_SECONDS - time_since_last:.1f}s remaining)")
            return False
    
    # Additional checks for obvious AI patterns
    if is_likely_ai_message(message_content):
        log_info(f"Skipping AI response - message appears AI-generated: {message_content[:50]}...")
        return False
...
def record_ai_message(message_content: str, platform: str, channel: str):
    ...
    log_info(f"Recorded AI message for {channel_key}: {message_content[:50]}...")
...
def cleanup_old_messages():
    ...
    for channel_key in recent_ai_messages:
        recent_ai_messages[channel_key] = {
            msg_hash for msg_hash in recent_ai_messages[channel_key]
            if msg_hash not in hashes_to_remove
        }
    
    if hashes_to_remove:
        log_info(f"Cleaned up {len(hashes_to_remove)} old message records")

def set_channel_cooldown(platform: str, channel: str, cooldown_seconds: float = None):
    ...
    log_info(f"Set cooldown for {channel_key}: {cooldown_seconds} seconds")
...
def get_channel_status(platform: str, channel: str) -> Dict[str, any]:
    ...
    if channel_key in cooldown_timers:
        ...
        status["last_response_time"] = datetime.fromtimestamp(cooldown_timers[channel_key]).isoformat()
...
def get_all_channel_stats() -> Dict[str, any]:
    ...
    for channel_key in recent_ai_messages:
        ...
        stats["channels"][channel_key] = channel_status
        
        if channel_status["on_cooldown"]:
            stats["channels_on_cooldown"] += 1
...
def reset_channel_tracking(platform: str, channel: str):
    ...
    log_info(f"Reset tracking for channel: {channel_key}")
...
def update_cooldown_settings(new_cooldown: float):
    ...
    log_info(f"Updated message cooldown from {old_cooldown}s to {new_cooldown}s")
...
def maintenance_cleanup():
    ...
    empty_channels = [
        channel_key for channel_key, messages in recent_ai_messages.items()
        if not messages
    ]
    ...
    if empty_channels:
        log_info(f"Removed {len(empty_channels)} empty channel entries") 